...
<header class="grid">
  <div>
    <h1>CineMatch</h1>
    <small class="muted">PRG + MVC + DAO (JDBC). Vistas sin lógica: JSTL + EL.</small>
  </div>
  <nav>
...
<main class="grid grid-2">
  ...
  <section class="card">
	...
	<div class="row">
        ...
        <select id="pelicula" name="pelicula" required>
          <c:forEach var="p" items="${pelicula}">
            <option value="${p.titulo}"></option>
          </c:forEach>
        </select>
      </div>
      <div class="row">
  ...
  <!-- Resultado de recomendación / panel lateral -->
  <aside class="card">
    <h3>Resultado</h3>
    ...
    <c:if test="${empty recomendacion}">
      <p class="muted">No hay recomendación disponible todavía. Elige “RECOMENDAR”.</p>
    </c:if>
...
<footer style="max-width:980px;margin:1rem auto;color:#666">
  <small>Controlador: <code>/cine</code> • PRG activo • Datos de apoyo esperados en request/session:
    <code>flash</code>, <code>generos</code>, <code>recomendacion</code>, <code>ultimas</code>, <code>aliasActual</code>, <code>generoActual</code>, <code>valoracionActual</code>.
  </small>
</footer>
</body>
</html>
